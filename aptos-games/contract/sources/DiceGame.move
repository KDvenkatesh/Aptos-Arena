module GameDapp::Games {
    ...
    struct RPSData has drop, store { result: u8 }                     // 0=draw,1=win,2=lose
    struct SnakeData has store { position: u8 }
    struct MysteryData has drop, store { last_prize: u8 }

    struct TreasureData has copy, store { chests_opened: u64, last_reward: u8, collection: vector<u8> }
    struct RacingData has copy, store { races_played: u64, wins: u64, last_vehicle: u8 }
    struct TimeRiftData has copy, store { runs: u64, best_time: u64, last_seed: u64 }
    struct BattleRoyaleData has copy, store { matches: u64, wins: u64, last_skin: u8 }

    struct TicTacToeData has copy, store { wins: u64, losses: u64, draws: u64 }
    struct HangmanData has copy, store { wins: u64 }
    struct MemoryMatchData has copy, store { games_won: u64, best_moves: u64 }
    struct FlappyCloneData has copy, store { high_score: u64, games_played: u64 }
